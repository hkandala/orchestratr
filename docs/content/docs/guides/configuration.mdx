...
each provider default requires a non-empty model and effort. orcr passes the resolved values
explicitly on every launch and records them on the agent. `defaults.*` is covered in
[choose providers and models](/docs/guides/providers-and-models). note there is no default timeout:
an agent never times out unless you pass `--timeout`.
