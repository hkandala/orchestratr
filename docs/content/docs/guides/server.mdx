...
- **integration state**: `integrations` shows which providers are built into orcr and whether their Herdr adapters are installed. a missing layer is why `agent run -a <p>` would fail with `integration_missing`. see [providers and integrations](/docs/concepts/providers).
