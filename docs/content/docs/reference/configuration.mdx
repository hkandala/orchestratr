...
| `defaults.providers.claude.model` | `"sonnet"` | one of `sonnet`, `opus`, `fable`, `haiku` |
| `defaults.providers.claude.effort` | `"medium"` | one of `low`, `medium`, `high`, `xhigh`, `max`, `ultracode` |
| `defaults.providers.codex.model` | `"gpt-5.6-luna"` | must appear in Codex app-server `model/list` |
| `defaults.providers.codex.effort` | `"high"` | must be advertised by the selected Codex model |
| `defaults.providers.<provider>.model` | (none) | required and non-empty for a configured custom provider |
| `defaults.providers.<provider>.effort` | (none) | required and non-empty for a configured custom provider |

`--model` and `--effort` override the selected provider's entries. orcr resolves both values
before enqueueing, passes both explicitly to the provider, and records the resolved pair on the
agent.

model and effort availability is checked at `agent run`, after CLI/config resolution and before
the agent is enqueued. invalid errors print the valid values and expose them as structured arrays.
