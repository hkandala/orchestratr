...
| new provider integration | implement `AgentIntegration` in a separate `src/driver/integration/<provider>.rs` module and register it in `integration.rs`; Herdr's adapter is still required (see [providers and integrations](/docs/concepts/providers)) |
